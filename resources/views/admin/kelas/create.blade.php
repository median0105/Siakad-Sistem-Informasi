            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('admin.kelas.store') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="mata_kuliah_id" name="mata_kuliah_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                            ...
                            @foreach ($mataKuliahs as $mataKuliah)
                                <option value="{{ $mataKuliah->id }}" @selected(old('mata_kuliah_id') == $mataKuliah->id)>
                    ...
                    <div>
                        ...
                        <x-text-input id="kode_kelas" name="kode_kelas" class="mt-1 block w-full" :value="old('kode_kelas')" required />
                    ...
                    <div class="grid gap-4 md:grid-cols-3">
                        <div>
                            ...
                            <x-text-input id="hari" name="hari" class="mt-1 block w-full" :value="old('hari')" />
                            <x-input-error class="mt-2" :messages="$errors->get('hari')" />
                        </div>

                        <div>
                            <x-input-label for="jam_mulai" value="Jam Mulai" />
                            <x-text-input id="jam_mulai" name="jam_mulai" type="time" class="mt-1 block w-full" :value="old('jam_mulai')" />
                            <x-input-error class="mt-2" :messages="$errors->get('jam_mulai')" />
                        </div>

                        <div>
                            <x-input-label for="jam_selesai" value="Jam Selesai" />
                            <x-text-input id="jam_selesai" name="jam_selesai" type="time" class="mt-1 block w-full" :value="old('jam_selesai')" />
                    ...
                    <div class="grid gap-4 md:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="ruangan" name="ruangan" class="mt-1 block w-full" :value="old('ruangan')" />
                            <x-input-error class="mt-2" :messages="$errors->get('ruangan')" />
                        </div>

                        <div>
                            <x-input-label for="kuota" value="Kuota" />
                            <x-text-input id="kuota" name="kuota" type="number" min="0" max="1000" class="mt-1 block w-full" :value="old('kuota', 0)" required />
